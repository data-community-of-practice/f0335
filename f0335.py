#!/usr/bin/env python3
"""
Normalise & Merge Crossref Authors (v3)
=========================================
...
Usage:
  python normalise_authors.py [Crossref_AuthorMetadata.json] [--output Normalised_Authors.json]
"""
...
def tokenise_given(given):
    ...
    Heuristic: a 2-3 letter token is treated as compressed initials
    if it contains NO vowels (a, e, i, o, u). If it has a vowel,
    it's likely a short name and kept as a single token.
    """
    # Strip periods, lowercase
    ...
    for p in parts:
        if len(p) <= 1:
            # Single character — always an initial
            result.append(p)
        elif len(p) <= 3 and all(c.isalpha() for c in p):
            has_vowel = any(c in vowels for c in p)
            if has_vowel:
                # Has a vowel — treat as a short name: "Wei", "Ian", "Ben"
                result.append(p)
            else:
                # All consonants — almost certainly initials: "SL", "DJ", "JJ"
                result.extend(list(p))
        else:
            # 4+ characters — always a name
            result.append(p)
...
def make_merge_key(given, family):
    ...
    Rules:
      - Periods stripped: "J." → "j"
      - Whitespace normalised
      - Compressed initials expanded: "JK" → "j k"
      - BUT full words stay as full words: "adam" stays "adam"
      - Order of parts preserved
    ...
    because "jane" ≠ "j". This is intentional — initial-to-full-name
    is a "similar" match, not auto-merge.
    """
...
def names_are_similar(given1, family1, given2, family2):
    ...
    Returns True if same family name and given names are compatible
    but not identical per merge key rules.

    Similar cases:
      - Initial vs full name: "J" ~ "Jane"
      - Missing parts: "Jane" ~ "Jane L"
      - Spelling variants: "Mathew" ~ "Matthew"
      - Swapped order: given1/family1 might be family2/given2
      - Hyphenated vs space-separated: "Anne-Marie" ~ "Anne Marie"
    """
    ...
    # Check family name match (including hyphen variants)
    fam_exact = (fam1 == fam2)
    fam_hyphen = _hyphen_equivalent(fam1, fam2)

    if fam_exact:
        if not tokens1 or not tokens2:
            return True  # One has no given name
        # Check if given name parts are compatible
        return _given_tokens_similar(tokens1, tokens2, given1, given2)

    if fam_hyphen:
        # Family names are hyphen variants — always flag as similar
        # regardless of given name match
        return True

    # Check surname-first swap: is fam1 == given2 parts and vice versa?
...
def _hyphen_equivalent(s1, s2):
    """
    Check if two strings are the same name in hyphenated vs
    space-separated form.
    "anne-marie" vs "anne marie" → True
    "mccarthy-jones" vs "mccarthy jones" → True
    """
    if s1 == s2:
        return False  # Identical, not just "similar"
    return s1.replace("-", " ") == s2.replace("-", " ")


def _given_tokens_similar(tokens1, tokens2, raw_given1="", raw_given2=""):
    """
    Check if two sets of given name tokens are similar.

    Similar means any of:
      - One is initial, other is full name starting with that letter
        ("j" ~ "jane")
      - One has fewer parts than the other
        (["jane"] ~ ["jane", "l"])
      - Spelling is close but not identical
        ("mathew" ~ "matthew")
      - Hyphenated vs space-separated
        ("anne-marie" ~ "anne marie")
    """
    # If merge keys would be the same, they're identical not similar
    if tokens1 == tokens2:
        return False

    # Check hyphenated variant of given names
    ...
    # Check if each token in shorter matches a token in longer
    # (initial-to-full, exact, or close spelling)
    matched_count = 0
    for sp in shorter:
        for lp in longer:
            if sp == lp:
    ...
    # At least one token must match for "similar"
    return matched_count > 0


def _spelling_similar(word1, word2):
    """
    Check if two words are close spelling variants.
    Uses simple edit distance — similar if ≤ 2 edits and both are
    full words (not initials).
    "johnathan" ~ "jonathon" → True
    "susan" ~ "sarah" → False
    """
...
# ============================================================
# SURNAME-FIRST DETECTION (Step A)
# ============================================================

def detect_and_fix_surname_first(appearances):
    """
    Detect entries where given/family are swapped.
    Only fixes high-confidence cases:
      - "family" field contains a period (e.g., "Jane L.")
    """
    fixed = 0
    for app in appearances:
        ...
        # If family has periods but given doesn't, it's likely swapped
        # E.g., given="Doe", family="Jane L."
...
def normalise_aff_name(name):
    """Normalise affiliation name for comparison."""
    n = name.lower()
...
def dedupe_affiliations(affiliations, threshold=0.80):
    ...
    clusters = []  # [(representative, [variants])]
    for aff in affiliations:
        ...
        for cluster in clusters:
            ...
            if aff_similarity(name, rep.get("name", "")) >= threshold:
                # Keep longer version, merge ROR/place
                if len(name) > len(rep.get("name", "")):
                    ror = aff.get("ror") or rep.get("ror")
                    ...
                    if not rep.get("ror") and aff.get("ror"):
                        rep["ror"] = aff["ror"]
...
def main():
    ...
    clusters = {}            # cluster_id -> {orcid, appearances[]}
    orcid_to_cluster = {}    # orcid -> cluster_id
    assigned = set()         # indices of appearances already assigned
    ...
    # Also try to merge non-ORCID appearances into ORCID clusters
    # if merge key matches exactly
    orcid_merge_keys = {}  # merge_key -> cluster_id
    ...
    # Group remaining by merge key
    mergekey_groups = defaultdict(list)  # merge_key -> [app]
    no_key = []
    ...
    for i, app in enumerate(appearances):
        ...
        mk = make_merge_key(app["given"], app["family"])
        if mk:
            # Check if this merge key matches an ORCID cluster
    ...
    # Create clusters for non-ORCID merge groups
    for mk, apps in mergekey_groups.items():
        cid = str(uuid.uuid4())
        clusters[cid] = {"orcid": None, "appearances": apps}

    # Create clusters for singletons with no merge key
    for app in no_key:
        cid = str(uuid.uuid4())
        clusters[cid] = {"orcid": None, "appearances": [app]}

    merged_into_orcid = sum(1 for i in assigned
                            if i < len(appearances)
                            and not appearances[i].get("orcid")
                            ) - len(no_key) - sum(len(apps) for apps in mergekey_groups.values())
    ...
    # Index by family name for efficient comparison
    ...
    # Also index by hyphen-stripped family name to catch
    # "McCarthy-Jones" vs "McCarthy Jones" across different groups
    family_dehyphen_index = defaultdict(set)  # dehyphenated -> set of original fam keys
    for fam in family_index:
        dehyph = fam.replace("-", " ")
        family_dehyphen_index[dehyph].add(fam)

    # Also index by given name (for surname-first detection)
    ...
    # Across hyphen-variant family name groups
    ...
    # Check surname-first swaps across different family names
    for fam, indices in family_index.items():
        # Is this "family name" also someone's given name?
    ...
    if multi_variant:
        ...
        for r in sorted(multi_variant, key=lambda x: len(x["name_variants"]), reverse=True)[:15]:
            ...
            print(f"  {r['full_name']}{tag}")
            print(f"    Variants: {r['name_variants']}")
        ...
        for r in researchers:
            for sim in r["similar_to"]:
                ...
                if pk not in shown:
                    ...
                    print(f"  \"{r['full_name']}\"{t1}  ~  \"{sim['full_name']}\"{t2}")
                    if len(shown) >= 30:
